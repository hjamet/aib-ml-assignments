...
def analyze_classification_models():
    ...
    X, y, _, _, _, _, _, _ = preprocess_data(
        df, selected_features, "Fill with median", True, "survived"
    )
    ...
    for model_name in available_models.keys():
        ...
        try:
            ...
            print(f"ERROR: {str(e)}")
            results.append(
                {"Model": model_name, "Time (s)": "ERROR", "Time_numeric": 999}
            )
    ...
    df_results = pd.DataFrame(
        [{k: v for k, v in r.items() if k != "Time_numeric"} for r in results]
    )
...
def analyze_regression_models():
    ...
    X, y, _, _, _, _, _, _ = preprocess_data(
        df, selected_features, "Fill with median", True, "fare"
    )
