...
def render_preprocessing_page(df):
    ...
    if "_target_column" not in st.session_state:
        st.session_state["_target_column"] = st.session_state.get(
    ...
    if not pd.api.types.is_numeric_dtype(df[target_column]):
        st.warning(
            f"⚠️ '{target_column}' is categorical. It will be encoded for Classification models."
        )

    # Categorical encoding method
    encoding_options = ["Drop Columns", "Label Encoding", "One-Hot Encoding"]
    if "_encoding_method" not in st.session_state:
        st.session_state["_encoding_method"] = st.session_state.get(
            "encoding_method", "Drop Columns"
        )
    ...
    if "_missing_age_option" not in st.session_state:
        st.session_state["_missing_age_option"] = st.session_state.get(
            "missing_age_option", "Fill with median"
        )
    if "_normalize_features" not in st.session_state:
        st.session_state["_normalize_features"] = st.session_state.get(
            "normalize_features", True
        )
    if "_selected_features" not in st.session_state:
        st.session_state["_selected_features"] = st.session_state.get(
            "selected_features", ["Age", "Sex", "Passenger Class", "Fare"]
        )
    ...
    target_feature_name = reverse_feature_mapping.get(target_column)
    available_features = [
        f
        for f in feature_options
        if f != target_feature_name and feature_mapping.get(f) != target_column
    ]
    ...
    valid_selected = [
        f for f in st.session_state["_selected_features"] if f in available_features
    ]
    if not valid_selected:
        valid_selected = [
            f for f in ["Age", "Passenger Class", "Fare"] if f in available_features
        ]
    ...
    # ===== TWO-COLUMN LAYOUT =====
    toc_markdown(
        '<h2 class="section-header">📊 Data Comparison</h2>',
        level=1,
        unsafe_allow_html=True,
    )
    ...
    if st.session_state.get("selected_features"):
        ...
        st.success(
            f"✅ Data preprocessing completed! Using {len(selected_features)} features: {', '.join(selected_features)}"
        )
    ...
    with col_left:
        ...
        if "_exploration_feature_left" not in st.session_state:
            st.session_state["_exploration_feature_left"] = st.session_state.get(
                "exploration_feature_left", "sex"
            )
        ...
        exploration_feature_left = st.selectbox(
            ...
            index=feature_options_explore.index(
                st.session_state["_exploration_feature_left"]
            ),
        ...
        if "survived" in df.columns:
            survival_by_feature = (
                df.groupby(exploration_feature_left)["survived"].mean().reset_index()
            )
            fig = px.bar(
                survival_by_feature,
                x=exploration_feature_left,
        ...
        st.info(
            f"💡 **Insight:** {insights.get(exploration_feature_left, 'Explore patterns in this feature!')}"
        )
    ...
    with col_right:
        ...
        if st.session_state.get("selected_features"):
            ...
            (
                ...
                missing_age_option,
                normalize_features,
                target_column=target_column_val,
                encoding_method=encoding_method_val,
            )
            ...
            display_preprocessing_results(
                before_count, after_count, len(selected_features), selected_features
            )
            ...
            if target_column_val in df_transformed_display.columns:
                df_transformed_display = df_transformed_display.rename(
                    columns={target_column_val: f"🎯 {target_column_val}"}
                )
            ...
            numeric_features = [
                col for col in df_transformed.columns if col != target_column_val
            ]
            ...
            if numeric_features:
                ...
                for feature in numeric_features:
                    ...
                    if "age" in feature_lower:
                        transformed_feature_labels[feature] = (
                        ...
                        transformed_feature_labels[feature] = feature.replace(
                            "_", " "
                        ).title()

                # Initialize temp key
                if "_exploration_feature_right" not in st.session_state:
                    st.session_state["_exploration_feature_right"] = (
                        st.session_state.get(
                            "exploration_feature_right", numeric_features[0]
                        )
                    )
                ...
                if target_column_val in df_transformed.columns:
                    target_by_feature = (
                        df_transformed.groupby(exploration_feature_right)[
                            target_column_val
                        ]
                        .mean()
                        .reset_index()
            ...
            st.info(
                "⚠️ Please select features in the configuration section above to see transformed data."
            )
